...
from utils.common_utils import init_normal, crop_image, get_image, pil_to_np, np_to_pil, plot_image_grid, get_noise, get_params, optimize, optimize_lr, np_to_torch, torch_to_np
...
def main(img: int=0, num_iter: int=40000, lr: float=3e-4, gpu: int=0, seed: int=42, save: bool=True):
    ...
    optimize(optimizer, closure_dip, 1000)#num_iter)
    ...
    parameters = get_params(OPT_OVER, net, net_input)
    optimizer = torch.optim.AdamW(parameters, lr=LR, weight_decay=weight_decay)
    optimize_lr(optimizer, closure_sgld, num_iter)
    ...
    parameters = get_params(OPT_OVER, net, net_input)
    optimizer = torch.optim.AdamW(parameters, lr=LR, weight_decay=weight_decay)
    optimize(optimizer, closure_mcdip, 1000)#num_iter)
